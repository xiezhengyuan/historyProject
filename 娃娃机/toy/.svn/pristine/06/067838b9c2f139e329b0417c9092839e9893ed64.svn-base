...
public class ToyServiceImpl implements ToyService {
	...
	@Override
	public Map<String, Object> toysInfoList(Employee em, int start, int limit, String agentname, String type) {
		List<Map<String, Object>> list = new ArrayList<Map<String, Object>>();
		Map<String, Object> resultMap = new HashMap<String, Object>();
		StringBuffer sql = new StringBuffer("SELECT COUNT(1) FROM toysinfo ti WHERE ti.state=0 and ");
		StringBuffer hql = new StringBuffer(
				"SELECT ti,tt FROM ToysInfo ti,ToysType tt WHERE ti.state=0 AND ti.ftoystypeid=tt.id");
		if(type.equals("1")){
			sql.append(" ti.empid = "+em.getId()+" ");
			hql.append(" and ti.empid="+em.getId()+"  ");
		}else{
			sql.append(" ti.empid in (select e.id from employee e  where e.name like '%"+agentname+"%' and e.role =1 and e.state =0  )");
			hql.append(" and ti.empid in (select e.id from Employee e  where e.name like '%"+agentname+"%' and e.role =1 and e.state =0  )");
		}
		...
		for (Object[] object : resultList) {
			...
			Employee emp=(Employee) databaseHelper.getObjectById(Employee.class, toysInfo.getEmpid());
	...
	@Override
	public void delToysType(Employee em, String id) throws Exception {
		String sql=" select count(id)  from  toysinfo where  state != -1 and ftoystypeid = "+id+" ";
		if(databaseHelper.getSqlCount(sql)>0) throw new Exception("有使用的娃娃绑定了该分类，不可删除");
	...
	@Override
	public Map<String, Object> findToysTypeById(Employee em, String id) throws Exception {
		Map<String, Object> map = new HashMap<String, Object>();
		ToysType toysType = (ToysType) databaseHelper.getObjectById(ToysType.class, Long.parseLong(id));
	...
	@Override
	public JsonArray findToysPhotos(Employee em, String toysid)throws Exception {
	...
	@Override
	public List<Map<String, Object>> querybannerimg()throws Exception {
	...
	@Override
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void updatebannerimgs(String imgs) throws Exception{
		//删除以前的图片
	...
	@Override
	@Transactional(propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public void deletetoys(String id) throws Exception {
		String sql=" select count(id) from machineinfo where state != -1 and ftoysid ="+id+" ";
		if(databaseHelper.getSqlCount(sql)>0) throw new Exception("改玩具绑定的娃娃机正在使用中，不可删除");
		
		//删除娃娃
		
		String sql1= " update toysinfo  set  state=-1 where id= "+id+"";
		databaseHelper.executeNativeSql(sql1);
		
		
		//删除娃娃图片
		
		String sql2="  update toysphotos set  state=-1 where ftoysid ="+id+" ";
		databaseHelper.executeNativeSql(sql2);
		
		
		//删除文件
		String sql3=" update fileinfo set  state=-1 where id in (select  ffileinfoid from toysphotos where ftoysid="+id+" ) ";
		databaseHelper.executeNativeSql(sql3);
