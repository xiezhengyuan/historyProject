...
//彭亮定义的Q方法
Q.gridbtn_xiangqing = function(tableid,dataid){
	return "<button title=\"用户详情\" class=\"btn btn-warning btn-xs\" style=\"color:#f60\" onclick=\"modify_"+tableid+"(" + dataid+ ")\">查看用户详情</button>";
}
Q.gridbtn_bohui = function(tableid,dataid){
	return "<button title=\"驳回申请\" class=\"btn btn-danger btn-xs\" style=\"color:#f60;margin-left:3px;\" onclick=\"del_"+tableid+"(" + dataid + ")\">驳回</button>";
}
Q.gridbtn_tongyi = function(tableid,dataid){
	return "<button title=\"同意申请\" class=\"btn btn-success btn-xs\" style=\"color:#f60;margin-left:3px;\" onclick=\"pass_"+tableid+"(" + dataid + ")\">同意</button>";
}

Q.gridbtn_xiangqing1 = function(tableid,dataid){
	return "<button title=\"代理详情\" class=\"btn btn-warning btn-xs\" style=\"color:#f60\" onclick=\"modify_"+tableid+"(" + dataid+ ")\">查看代理详情</button>";
}
Q.gridbtn_bohui1 = function(tableid,dataid){
	return "<button title=\"取消代理\" class=\"btn btn-danger btn-xs\" style=\"color:#f60;margin-left:3px;\" onclick=\"del_"+tableid+"(" + dataid + ")\">取消代理</button>";
}
//结束
...
Q.gridbtn_bindtoy = function(tableid,dataid){
	return "<button title=\"绑定玩具\" class=\"btn btn-success btn-xs\" style=\"color:#f60;margin-left:3px;\" onclick=\"bind_"+tableid+"(" + dataid + ")\"><span class=\"glyphicon glyphicon-edit\" aria-hidden=\"true\"></span>绑定玩具</button>";
}

Q.gridbtn_pass = function(tableid,dataid){
	return "<button title=\"发货\" class=\"btn btn-success btn-xs\" style=\"color:#f60;margin-left:3px;\" onclick=\"pass_"+tableid+"(" + dataid + ")\">通过</button>";
}
...
//帖子分类
function querypostingstyle(selectedid){
	$._ajax({
		url: Q.url+"/posting/querypostingstyle.action",
		data:{},
		async:false,
		success: function(data){
			if( typeof(data) == 'object' && data.op == 'timeout' ){
...
Q.myconfirm = function(message,fn){
...
Q.toystype = null;//缓存玩具类型数据

Q.getToystype = function(selecter){
	//检查缓存中是否有玩具类型信息
	if(Q.toystype!=null){
		var opts = '<option value="0">请选择</option>';
		for(var i=0;i<Q.toystype.rows.length;i++){
			var type = Q.toystype.rows[i];
	...
	$._ajax({
		url: Q.url+"/toy/findAllToysType.action",
		data:{},
		async:false,
		success: function(data){
			if( typeof(data) == 'object' && data.op == 'timeout' ){
...
Q.getToystype1 = function(selecter){
	//检查缓存中是否有玩具类型信息
	if(Q.toystype!=null){
		var opts = '<option value="0">请选择</option>';
		for(var i=0;i<Q.toystype.rows.length;i++){
	...
	$._ajax({
		url: Q.url+"/device/findAllToysType.action",
		data:{},
		async:false,
		success: function(data){
			if( typeof(data) == 'object' && data.op == 'timeout' ){
...
Q.getToys = function(selecter){
	//检查缓存中是否有玩具信息
	if(Q.toys!=null){
		var opts = '<option value="0">请选择</option>';
		for(var i=0;i<Q.toys.rows.length;i++){
			var toys = Q.toys.rows[i];
			opts+= '<option value="'+toys.id+'">'+toys.name+'</option>';
		}
		$(selecter).html(opts);
