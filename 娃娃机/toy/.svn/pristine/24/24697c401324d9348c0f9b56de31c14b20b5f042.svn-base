<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
	...
	<button id="addios" class="btn btn-purple btn-sm" onclick="addios()" data-placement="top" title="更新ios版本" data-toggle="modal" data-target="#adduser">
...
Q.grid_selector = "order_grid_table1";
Q.pager_selector = "order_grid_pager1";
...
var colModel = [
                ...
                {  name: '操作' ,align :'center', fixed: true, sortable: false, resize: false, width:100,  
                    ...
                    formatter: function (value, grid, rows, state) { 
                    	...
                    	var btn = "";
                    	if(rows.state=='停用'){
                    		btn += "<button title=\"使用该版本\" class=\"btn btn-warning btn-xs\" style=\"color:#f60\" onclick=\"modify_order_grid_table1(" +rows.id+")\"><span class=\"glyphicon glyphicon-pencil\" aria-hidden=\"true\"></span></button>"
...
/* function addios(){
	var _url= 'edition/addnewedition.jsp?type=2';
	
	Q.viewJsp('#div_addios',_url);
} */

function modify_order_grid_table1(dataid){
	Q.confirm("运行该版本将导致其他版本停止使用，确定使用该版本么？",function(r){
		if(r){
			$._ajax({
				url: Q.url+"/edition/usethisedition.action",
				data:{"id":dataid,"type":"2"},
