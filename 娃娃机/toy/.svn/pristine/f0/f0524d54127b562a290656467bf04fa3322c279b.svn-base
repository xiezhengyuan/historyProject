...
<ul class="nav nav-tabs" role="tablist">
  ...
  <li style="position: relative;float: right;">
    发货人
  <select id="empid" onchange="queryorderbycondition()">
  
  </select>
  <input type="text" id="toyname"  placeholder="玩具名称"/>
  <input type="text" id="keyword"  placeholder="申请人姓名或手机号 "/>
...
var colModel = [
                ...
                { label: '待发货娃娃', name: 'toysname',align :'center', sortable : false, width:60  },
                { label: '娃娃图片', name: 'toyurl',align :'center', sortable : false ,width:60,
                	formatter: function (value, grid, rows, state) { 
                		return '<img src = "'+Q.url+value+'" width="30%"   >';
                	}
                },
                { label: '需发货人', name: 'empname',align :'center', sortable : false ,width:60 },
...
function pass_order_grid_table(id){
	var _url= 'deliveryapply/deliverypass.jsp?id='+id;
...
function del_order_grid_table(id){
	var type;
	Q.myconfirm("是否将该用户礼物盒中的数据也删除？",function(r){
		if(r){
			type='1';
		}else{
			type='2';
		...
		$._ajax({
			url: Q.url+"/ServiceDeliveryapplyControl/deleteapp.action",
...
function modify_order_grid_table(id){
    var _url= 'deliveryapply/sendtouser.jsp?id='+id;
