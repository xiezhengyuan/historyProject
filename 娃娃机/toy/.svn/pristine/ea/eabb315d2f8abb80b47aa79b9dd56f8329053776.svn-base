...
public class ModelConfigControl {
	...
	*/
		...
		public void mysocket(HttpServletRequest request,HttpSession session,HttpServletResponse response){
			...
			try {
				...
				while (it.hasNext()) {
					...
					if (entry.getKey().equals(key)) {
						Sys.out("delete this key: " + key + " = " + entry.getValue());
						it.remove(); // OK
					}
				}
				
				JsonUtil.success2page(response, e.getMessage());
			}
		}
