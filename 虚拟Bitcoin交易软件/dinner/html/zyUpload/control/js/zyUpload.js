		...
		return this.each(function(){
			...
			this.funFilterEligibleFile = function(files){
				...
				for (var i = 0, file; file = files[i]; i++) {
					...
					//	Q.alert('提示','您这个"'+ file.name +'"文件大小过大');	
					//} else {
						// 在这里需要判断当前所有文件中
						arrFiles.push(file);	
					//}
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				// 处理不同类型文件代表的图标
				var fileImgSrc = "html/zyUpload/control/images/fileType/";
				...
				}else if(file.type.indexOf("jpeg") > 0||file.type.indexOf("jpg") > 0||file.type.indexOf("png") > 0||file.type.indexOf("image") > 0){
					fileImgSrc = fileImgSrc + "image.png";
				}else{
					fileImgSrc = fileImgSrc + "file.png";
				}
				
				//Q.alert("提示",'file.type.indexOf("image") == 0...'+(file.type.indexOf("image") == 0));
				// 图片上传的是图片还是其他类型文件
				if (file.type.indexOf("image") == 0&&e!=null) {
			...
			this.createCorePlug = function(){
				var params = {
						...
						var funDealtPreviewHtml = function() {
							...
							if (file) {
								if(file.size<5*1024*1024){
									...
									html += self.funDisposePreviewHtml(file, null);
					...
					onDelete: function(file, files) {
						//Q.alert('提示',file.name);
						Q.filesarr.remove(file.name);
						// 移除效果
					...
					onSuccess: function(file, response) {
						if(JSON.parse(response).succ){
							...
							Q.filesarr[Q.filesarr.length] = {"id":JSON.parse(response).fileId,"url":JSON.parse(response).url,"sourcename":JSON.parse(response).sourcename};
							// 根据配置参数确定隐不隐藏上传成功的文件
							if(para.finishDel){
								// 移除效果
								$("#uploadList_" + file.index).fadeOut();
