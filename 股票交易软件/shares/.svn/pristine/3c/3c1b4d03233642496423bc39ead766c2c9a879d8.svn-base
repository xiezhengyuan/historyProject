...
public class CompanyServiceImpl implements CompanyService{
	...
	public void forbiddencompany(String id, Integer state) throws Exception {
		// TODO Auto-generated method stub
		//AccountInfo accountInfo = databaseHelper.getObjectById(AccountInfo.class, Long.parseLong(id))
		
		Company company = (Company) databaseHelper.getObjectById(Company.class, Long.parseLong(id));
		StringBuffer queryexcist = new StringBuffer("select count(a.id) from AccountInfo a where a.fcompanyid = ")
				.append(Long.parseLong(id));
		List lst = databaseHelper.getResultListByHql(queryexcist.toString());
		Integer excist = Integer.parseInt(lst.get(0).toString());
		//判断该公司名下是否有员工 用户，有则需要先转移再禁用。
		if(excist==0){
		company.setState(state);
		databaseHelper.updateObject(company);
		}else{
			throw new Exception("先转移该公司下的员工才能禁用");
		}
	}
	...
	public void forbiddenmember(String id, Integer state) throws Exception {
		...
		List lst = databaseHelper.getResultListByHql(queryexcist.toString());
		Integer excist = Integer.parseInt(lst.get(0).toString());
		//判断该员工名下是否有 用户，有则需要先转移再禁用。
		if(excist==0){
			accountInfo.setState(state);
		databaseHelper.updateObject(accountInfo);
		}else{
			throw new Exception("先转移该员工下的用户才能禁用");
		}
	}
