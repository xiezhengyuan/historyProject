...
public class AppServiceUserInfoControl {
	...
	public void mygoldsdetialanddetial(HttpServletRequest request,HttpServletResponse response){
		try {
			...
			String inorout=request.getParameter("inorout")==null?"0":request.getParameter("inorout");
			...
			int limit = request.getParameter("rows")==null?ConstantUtil.LIMIT:Integer.parseInt(request.getParameter("rows"));
			List<Map<String,Object>> lstMap = new ArrayList<Map<String,Object>>();
			int records = appServiceUserinfo.countmygoldsdetialanddetial(Long.parseLong(userid),type,time,starttime,endtime,inorout);
			int total = ConstantUtil.pages(records, limit);
			...
			if(records > 0 ){
				lstMap = appServiceUserinfo.mygoldsdetialanddetial(Long.parseLong(userid),type,time,starttime,endtime, start, limit,inorout);
			}	
			String json = new Gson().toJson(lstMap);
			JsonArray arr = (JsonArray) new JsonParser().parse(json);
			JsonObject obj = new JsonObject();
			...
			if(inorout.equals("in")){
				...
				obj.addProperty("totaloutgolds",0);
			}else if(inorout.equals("out")){
				obj.addProperty("totalingolds",0);
	...
	public void withdraw(HttpServletRequest request,HttpServletResponse response){
		try {
			...
			JsonUtil.success2page(response, "{\"op\":\"fail\",\""+e.getMessage()+"\":\"服务器忙\"}");
			e.printStackTrace();
		}
	}
	...
	@RequestMapping(method = RequestMethod.POST, value = "/pcupdatemyinfo")
	public void pcupdatemyinfo(HttpServletRequest request,HttpServletResponse response){
		try {
			String userid = request.getParameter("userid")!=null&&request.getParameter("userid").length()>0?request.getParameter("userid"):"";
			String nickname=request.getParameter("nickname")!=null&&request.getParameter("nickname").length()>0?request.getParameter("nickname"):"";
			String name=request.getParameter("name")!=null&&request.getParameter("name").length()>0?request.getParameter("name"):"";
			String mobile=request.getParameter("mobile")!=null&&request.getParameter("mobile").length()>0?request.getParameter("mobile"):"";
			String email=request.getParameter("email")!=null&&request.getParameter("email").length()>0?request.getParameter("email"):"";
		    String intro=request.getParameter("intro")!=null&&request.getParameter("intro").length()>0?request.getParameter("intro"):"";
			appServiceUserinfo.pcupdatemyinfo(Long.parseLong(userid),nickname, name, mobile, email,intro);
			JsonUtil.success2page(response);
		} catch (Exception e) {
			JsonUtil.success2page(response, "{\"op\":\"fail\",\"msg\":\"服务器忙\"}");
			e.printStackTrace();
	...
	@RequestMapping(method = RequestMethod.POST, value = "/updatemybank")
	public void updatemybank(HttpServletRequest request,HttpServletResponse response){
		try {
			String userid = request.getParameter("userid");
			...
			String cardno=request.getParameter("cardno");
			String paparno=request.getParameter("paparno");
			appServiceUserinfo.updatemybank(Long.parseLong(userid),bankaccount,bank,cardno,paparno );
			JsonUtil.success2page(response);
		} catch (NullPointerException e) {
			JsonUtil.success2page(response, "{\"op\":\"fail\",\"msg\":\""+e.getMessage()+"\"}");
		} catch (Exception e) {
