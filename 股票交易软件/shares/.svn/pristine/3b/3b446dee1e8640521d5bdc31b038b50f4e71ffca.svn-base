...
public class ExampleServiceImpl implements ExampleService {
	...
	public Map<String, Object> findExampleList(AccountInfo ai, String keyword, int start, int limit) {
		...
		StringBuffer hql = new StringBuffer("SELECT ui FROM UserInfo ui WHERE ui.isexample=1 AND ui.state!=-2 ");
		if (!StringUtils.isEmpty(keyword)) {
		...
		for (Object object : resultList) {
			...
			map.put("time", ui.getCreatetime().toString());
			if (ui.getState() == 0) {
				map.put("state", "正常");
			} else {
				map.put("state", "封禁");
	...
	public Map<String, Object> panicBuying(AccountInfo ai, String id, String type, int start, int limit) {
		...
		for (Object object : resultList) {
			...
			map2.put("startTime", ep.getStarttime().toString());
			map2.put("endTime", ep.getEndtime().toString());
			map2.put("time", ep.getCreatetime().toString());
	...
	@Override
	public Map<String, Object> planRunning(AccountInfo ai, String id,String type) {
		Map<String, Object> resultMap = new HashMap<String, Object>();
		StringBuffer hql = new StringBuffer("SELECT ep FROM ExamplePlan ep WHERE ep.state=1 AND ep.fuserinfoid=").append(id)
				.append(" AND ep.type=").append(type).append(" ORDER BY ep.createtime DESC");
		...
		if(resultList!=null && resultList.size()!=0){
			...
			resultMap.put("starttime", plan.getStarttime().toString());
			resultMap.put("endtime", plan.getEndtime().toString());
			...
			resultMap.put("op", "empty");
			resultMap.put("msg", "暂无运行中计划");
		}
		return resultMap;
	}
