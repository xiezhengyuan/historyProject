...
public class ManagerServiceImpl implements ManagerService {
	...
	public Map<String, Object> queryAll2(Integer start, Integer limit, String type,String keyword) throws Exception {
		...
		List<Map<String, Object>> lstmap=new ArrayList<Map<String,Object>>();
		StringBuffer buffer=new StringBuffer("select count(s.id) from shares s where   s.state > -2 and s.type=").append(type);
		if(keyword!=null&&keyword.length()>0)buffer.append(" and s.sharesname like '%").append(keyword).append("%'");
		int records=databaseHelper.getSqlCount(buffer.toString());
		int total=ConstantUtil.pages(records, limit);
		if(records>0){
			StringBuffer buffer1=new StringBuffer("select s from Shares s where s.state > -2 and s.type=").append(type);
			if(keyword!=null&&keyword.length()>0)buffer.append(" and s.sharesname like '%").append(keyword).append("%'");
