...
public class PermissionsServiceImpl implements PermissionsService {
	...
	public Map<String, Object> findPermissionsInfo(AccountInfo ai, int start, int limit) {
		...
		Integer role = ai.getRole();
		if (role == 0 || role == 1) {
			hql.append("SELECT ai FROM AccountInfo ai WHERE ai.role=0 OR ai.role=1");
			sql.append("SELECT COUNT(*) FROM accountinfo ai WHERE ai.role=0 OR ai.role=1");
		} else {
			hql.append("SELECT ai FROM AccountInfo ai WHERE (ai.role=2 OR ai.role=3) AND ai.fcompanyid=")
					.append(ai.getFcompanyid());
			sql.append("SELECT COUNT(*) FROM accountinfo ai WHERE (ai.role=2 OR ai.role=3) AND ai.fcompanyid=")
					.append(ai.getFcompanyid());
		}
		...
		for (Object object : resultList) {
			...
			map.put("createtime", info.getCreatetime().toString());
	...
	public Map<String, Object> addPermissions(AccountInfo ai, String id, String username, String password,
		...
		if (StringUtils.isEmpty(id)) {
			...
			if (ai.getRole() == 0 || ai.getRole() == 1) {
				accountInfo.setType(1);
				accountInfo.setRole(1);
			} else {
				accountInfo.setType(2);
				accountInfo.setRole(3);
				accountInfo.setFcompanyid(ai.getFcompanyid());
				accountInfo.setProportion(ai.getProportion());// 公司分成比例
			}
			accountInfo.setCreatetime(new Date());
			accountInfo.setState(0);
			accountInfo.setDefaultaccount(0);
			...
			if (!StringUtils.isEmpty(menu)) {
				...
				for (String menuid : ids) {
					AccountPermission permission = new AccountPermission();
					permission.setFaccountid(Long.parseLong(id));
					permission.setFmenuid(Long.parseLong(menuid));
					permission.setState(0);
					databaseHelper.persistObject(permission);
				}
	...
	public Map<String, Object> queryMenu(AccountInfo ai, String id) {
		...
		resultMap.put("defaultaccount", ai.getDefaultaccount());
		List<Map<String, Object>> list = new ArrayList<Map<String, Object>>();
		if (StringUtils.isEmpty(id)) {
			// 平台管理员
